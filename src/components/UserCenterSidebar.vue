...
const navGroups = computed(() => [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '搜索历史', to: '/user/search-history' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '卖家认证', to: '/user/security' },
      { label: '我的钱包', to: '/user/wallet' },
    ],
  },
])

function isActive(item) {
  return item.exact ? route.path === item.to : route.path === item.to
}
</script>
...
<template>
  <Card class="w-56 shrink-0 rounded-[28px] border border-slate-200/80 bg-white p-4 shadow-sm">
    <div class="space-y-5">
      <div v-for="(group, groupIndex) in navGroups" :key="`group-${groupIndex}`" class="space-y-2">
        <p
          v-if="group.title"
